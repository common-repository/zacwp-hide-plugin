...
class ZacWP_PO_Plugin_Order_Hide {
	...
	public $plugin_columns = ['id','slug','plugin','new_version','url','package','icons','banners',
		...
		'Plugin_Size', 'Last_Modified', 'Plugin_Installed', 'Plugin_File', 'last_activated', 'last_deactivated', 'Note'];
	...
	public function __construct() {
		...
		add_action("wp_ajax_zacwp_po_ajax_request", array($this, "zacwp_po_ajax_request"));


	}

	public function zacwp_po_ajax_request() {
		if ( wp_verify_nonce( sanitize_text_field($_REQUEST['zacwp_po_note_nonce']), "zacwp_po_ajax_request")
            && isset($_POST['zacwp_po_plugin'])
            && isset($_POST['zacwp_po_note_value'])
        ) {
		    $plugin = sanitize_text_field($_POST['zacwp_po_plugin']);
		    $plugin_note = sanitize_text_field($_POST['zacwp_po_note_value']);

			$this->pluginsLog[ $plugin ]['note'] = $plugin_note;
	...
	public  function zacwp_po_append_scripts() {
        ...
        <script>
            function submitNote(id) {
                ...
                formData.append("zacwp_po_plugin", $(`#${id}_plugin`).val());
                formData.append("zacwp_po_note_value", $(`#${id}_note`).val());

                console.log(JSON.stringify(formData, null, 4));
                //ajax
                $.ajax({
                    url: "<?php
                        $action = "zacwp_po_ajax_request";
                        $nonce = wp_create_nonce($action);
				        echo admin_url("admin-ajax.php?action=$action&zacwp_po_note_nonce=$nonce");
				        ?>",
                    type: "POST",
                    data: formData,
                    contentType: false,
	...
	public function zacwp_plugins_extra_columns( $column_name, $plugin_file, $plugin_data ) {
		...
		switch ($column_name) {
			...
			case 'Note':
				$note = '';
				if(array_key_exists($plugin_file, $this->pluginsLog)
				      && array_key_exists('note', $this->pluginsLog[$plugin_file] )) {
				    $note = $this->pluginsLog[$plugin_file]['note'];
                }
				$explode = explode("/", $plugin_file);
				$id = "zacwp_po_".$explode[0];
				?>
                <span id="<?php echo $id; ?>" >
                    <input type="hidden" id="<?php echo $id; ?>_plugin" value="<?php echo $plugin_file; ?>">
                    <textarea class="large-text code" placeholder="Enter note" id="<?php echo $id; ?>_note"><?php echo $note; ?></textarea>
                    <input class="button button-primary" value="Update" onclick="submitNote('<?php echo $id; ?>')">
                </span>
                <?php
				break;
			default;
	...
	public function zacwp_po_settings() {
		...
		if ( current_user_can( 'administrator' ) ) {
			...
			$url = $this->zacwp_po_get_current_url();
	...
	public function zacwp_po_get_current_url() {
